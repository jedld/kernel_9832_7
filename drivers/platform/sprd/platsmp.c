...
static int __cpuinit boot_secondary_cpus(int cpu_id, u32 paddr)
{
	...
	if (cpu_id < 1 || cpu_id > 3)
		return -1;
#ifdef CONFIG_TRUSTY
	writel(0x96000000, (CPU_JUMP_VADDR + (cpu_id << 2)));
#else
	writel(paddr,(CPU_JUMP_VADDR + (cpu_id << 2)));
#endif
	writel(readl(HOLDING_PEN_VADDR) | (1 << cpu_id),HOLDING_PEN_VADDR);
#ifdef CONFIG_TRUSTY
	ret = trusty_fast_call32_power(SMC_FC_CPU_UP, paddr, cpu_id, 0);
	if (ret < 0)
		pr_warn("trusty_fast_call32_power: SMC_FC_CPU_UP failed!, ret = %d\n", ret);
#endif
	poweron_cpus(cpu_id);
