...
static void sprdfb_dispc_clean_lcd (struct sprdfb_device *dev)
{
	...
	uint32_t base = fb->fix.smem_start + (fb->fix.line_length * var->yoffset);
...
#ifdef CONFIG_FB_SC9001
	disp_pw_on(DISP_PW_DOMAIN_DISPC);
	clk_prepare_enable(dispc_ctx.clk_dispc_emc);
	dispc_early_configs();
#endif
	if (dev->enable == 0) {
...
//#define USE_OVERLAY_BUFF
	...
	logo_size = dev->panel->width * dev->panel->height * 3;// should be rgb88
#else
