...
static void gpio_keys_gpio_report_event(struct gpio_button_data *bdata)
{
	...
	if (type == EV_ABS) {
		...
		if (bdata->last_state == state) {
			input_event(input, type, button->code, !state);
			PRINT_INFO("last event Key:%s ScanCode:%d value:%d\n", button->desc, button->code, !state);
		}
...
static irqreturn_t gpio_keys_gpio_isr(int irq, void *dev_id)
{
	...
	if (irqd_get_trigger_type(irq_get_irq_data(irq)) != bdata->irqflags) {
		PRINT_INFO("wake up cpu from deepsleep by gpio level interrupt!");
				...
				const struct gpio_keys_button *button)
{
	...
	if (gpio_is_valid(button->gpio)) {
		...
		bdata->is_deepsleep = 0;
		bdata->last_state = 0xFF;/* set default invalid ,not 0/1 */
	} else {
		if (!button->irq) {
...
gpio_keys_get_devtree_pdata(struct device *dev)
{
	...
	for_each_child_of_node(node, pp) {
		...
		if(!button->ds_irqflags)
			button->ds_irqflags = IRQF_TRIGGER_LOW;//default
		else if(button->ds_irqflags)
			button->ds_irqflags = IRQF_TRIGGER_HIGH;
		PRINT_INFO("desc:%s,button->ds_irqflags = 0x%08x\n",button->desc, button->ds_irqflags);
	}
