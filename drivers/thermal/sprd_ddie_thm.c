...
#define RAW_DATA_LOW         (618)
#define RAW_DATA_HIGH        (1070)
#else
...
#define SHARKLC_RAW_DATA_LOW         (624)
#define SHARKLC_RAW_DATA_HIGH        (1075)
...
int sprd_sharklc_rawdata2temp( int rawdata)
{
	int temp_result;
	if (rawdata < SHARKLC_RAW_DATA_LOW) {
			rawdata = SHARKLC_RAW_DATA_LOW ;
		}
	if (rawdata > SHARKLC_RAW_DATA_HIGH) {
			rawdata = SHARKLC_RAW_DATA_HIGH ;
		}
	temp_result = TEMP_LOW +
		(rawdata - SHARKLC_RAW_DATA_LOW) * (TEMP_HIGH - TEMP_LOW) / (SHARKLC_RAW_DATA_HIGH - SHARKLC_RAW_DATA_LOW);
...
static unsigned long sprd_thm_temp_read(struct sprd_thermal_zone *pzone)
{
	...
	if (pzone->tmp_flag){
		if(soc_is_scx9832a_v0()){
			temp = sprd_sharklc_rawdata2temp(rawdata) - cal_offset;
		}else{
                         ...
                         struct device *dev)
{
	...
	thm_data =devm_kzalloc(dev, sizeof(*thm_data), GFP_KERNEL);
	if (!thm_data) {
		dev_err(dev, "could not allocate memory for platform sprd_thm\n");
		return NULL;
	}
	thm_data->nsensor = nsensor;
	pdata = devm_kzalloc(dev, thm_data->nsensor*sizeof(*pdata), GFP_KERNEL);
	if (!pdata) {
