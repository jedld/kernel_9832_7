...
static int mapped_ratio = 60;
static int cached_ratio = 18;
static int freeswap_ratio = 5;
/* cached app(adj 9)=>529 in M,and 900 in N */
static int mem_pressure_min_score_adj = 900;
...
static bool mem_pressure(void)
{
	...
	cached = global_page_state(NR_FILE_PAGES) -
				total_swapcache_pages() - mem.bufferram;
	mapped = global_page_state(NR_FILE_MAPPED);

	if (cached < 0)
		cached = 0;

	if (100 * cached >= cached_ratio * mem.totalram)
		return false;

	si_swapinfo(&mem);

	if (100 * mem.freeswap >= freeswap_ratio * mem.totalswap)
		return false;

	if (100 * mapped < mapped_ratio * cached)
		return false;

	lowmem_print(1, "enter mem_pressure.cached/totalram=%%%ld,\n"
			"freeswap/totalswap=%%%ld,mapped/cached=%%%ld\n",
			100*cached/mem.totalram, 100*mem.freeswap/mem.totalswap,
			100*mapped/cached);
...
static int lowmem_shrink(struct shrinker *s, struct shrink_control *sc)
{
	...
	if (nr_to_scan <= 0 || min_score_adj == OOM_SCORE_ADJ_MAX + 1) {
		if (nr_to_scan > 0 && mem_pressure()) {
			min_score_adj = mem_pressure_min_score_adj;
			lowmem_print(2, "lowmem_scan, has memory pressure.\n"
					"adjust adj to %d\n", min_score_adj);
		} else {
...
static uint32_t oom_score_to_oom_enable = 0;
...
#endif
module_param_named(cached_ratio, cached_ratio, int, S_IRUGO | S_IWUSR);
module_param_named(mapped_ratio, mapped_ratio, int, S_IRUGO | S_IWUSR);
module_param_named(freeswap_ratio, freeswap_ratio, int, S_IRUGO | S_IWUSR);
module_param_named(mem_pressure_min_score_adj, mem_pressure_min_score_adj, int, S_IRUGO | S_IWUSR);
