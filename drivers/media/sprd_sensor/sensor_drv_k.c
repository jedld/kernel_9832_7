...
LOCAL int _sensor_k_rd_i2c_seq(struct sensor_file_tag *fd_handle, struct sensor_i2c_seq_tag *pI2cTab)
{
	char              *pBuff = PNULL;
	int                ret = SENSOR_K_FAIL;
	int                i;
	...
	/*alloc buffer */
	pBuff = _sensor_k_malloc(fd_handle, pI2cTab->i2c_count);
	if (PNULL == pBuff) {
		ret = SENSOR_K_FAIL;
		SENSOR_PRINT_ERR("sensor rd I2C ERR: alloc fail, size %d\n", pI2cTab->i2c_count);
		goto sensor_k_readi2c_return;
	...
	if (copy_from_user(pBuff, pI2cTab->i2c_data, pI2cTab->i2c_count)) {
		ret = SENSOR_K_FAIL;
		SENSOR_PRINT_ERR("sensor W I2C ERR: copy user fail, size %d \n", pI2cTab->i2c_count);
		goto sensor_k_readi2c_return;
	}
	...
	for (i = 0; i < SENSOR_I2C_OP_TRY_NUM; i++) {
		...
		msg_r[0].flags = 0;
		msg_r[0].buf = pBuff;
		msg_r[0].len = pI2cTab->wr_cnt;
		msg_r[1].addr =  pI2cTab->slave_addr;
		msg_r[1].flags = I2C_M_RD;
		msg_r[1].buf = pBuff;
		msg_r[1].len = pI2cTab->rd_cnt;
#if 1
		if (NULL == p_mod->sensor_dev_tab[fd_handle->sensor_id].vcm_i2c_client) {
			i2c_client = p_mod->sensor_dev_tab[fd_handle->sensor_id].cur_i2c_client;
		} else {
		...
		if (ret != 2) {
			...
			ret = SENSOR_K_SUCCESS;
			if (copy_to_user(pI2cTab->i2c_data, pBuff, pI2cTab->rd_cnt)) {
				ret = SENSOR_K_FAIL;
				SENSOR_PRINT_ERR("sensor W I2C ERR: copy user fail, size %d \n", pI2cTab->i2c_count);
				goto sensor_k_readi2c_return;
			}
				...
				unsigned long arg)
{
	...
	case SENSOR_IO_I2C_READ_EXT:
		...
		break;
	case SENSOR_IO_I2C_READ_SEQ:
		{
			struct sensor_i2c_seq_tag i2cTab;
			SENSOR_PRINT("SENSOR: ioctl SENSOR_IO_I2C_READ_SEQ \n");
			ret = copy_from_user(&i2cTab, (struct sensor_i2c_seq_tag*) arg, sizeof(struct sensor_i2c_seq_tag));
			if (0 == ret)
				ret = _sensor_k_rd_i2c_seq(p_file, &i2cTab);
		}
		break;
